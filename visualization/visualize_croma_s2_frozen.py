"""
Visualization script for CROMA PASTIS S2 Frozen model.

Generates detailed visualizations and analysis of predictions from the frozen CROMA model
trained on PASTIS with Sentinel-2 modality, including:
...
Usage:
    ...
    python visualize_croma_s2_frozen.py --dataset-root /mnt/tania/pastis-r

    # Custom runs
    python visualize_croma_s2_frozen.py --num-samples 32 --split val --dataset-root /mnt/tania/pastis-r
    python visualize_croma_s2_frozen.py --split test --output-dir ./custom_viz --dataset-root /mnt/tania/pastis-r
    python visualize_croma_s2_frozen.py --dataset-root /path/to/PASTIS --no-grid
"""
...
# Fixed paths for croma_pastis_s2_frozen
BASE_OUTPUT_DIR = Path('/mnt/tania/geobreeze/outputs/croma_pastis_s2_frozen')
CHECKPOINT_PATH = BASE_OUTPUT_DIR / 'lr=6.25e-06_bsz=16_e=50/checkpoints/best_model-epoch=49.ckpt'
...
def load_lightning_task(checkpoint_path: str, device: str):
    ...
    model_cfg = cfg.model
    logger.info(f"Building CROMA encoder: {model_cfg.hf_filename}")
    encoder = Croma(
...
def plot_sample_detailed(rgb, gt, pred, confidence, patch_id, out_path):
    ...
    fig.suptitle(f'PASTIS Patch {patch_id} · CROMA Prediction (S2)',
...
def plot_comparison_grid(samples, out_path):
    ...
    fig.suptitle('PASTIS · CROMA Predictions (S2) | GT vs Pred',
...
def plot_class_performance(all_ious, all_f1s, out_path):
    ...
    fig.suptitle('Per-Class Performance (CROMA on PASTIS S2)',
...
def plot_confusion_matrix(all_gt, all_pred, out_path):
    ...
    ax.set_title('Confusion Matrix (CROMA on PASTIS S2)', color='white', fontsize=13)
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualize CROMA PASTIS S2 Frozen model predictions',
    ...
    if 'MODEL_WEIGHTS_DIR' not in os.environ:
        os.environ['MODEL_WEIGHTS_DIR'] = str(Path(__file__).parent / 'fm_weights')
