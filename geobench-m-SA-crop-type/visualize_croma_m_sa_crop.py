"""
Visualization script for CROMA South Africa Crop Type model.

Generates detailed visualizations and analysis of predictions from the CROMA model
...
Usage:
    ...
    python visualize_croma_m_sa_crop.py

    # Custom runs
    python visualize_croma_m_sa_crop.py --num-samples 32 --split valid
    python visualize_croma_m_sa_crop.py --split test --output-dir ./custom_viz
    python visualize_croma_m_sa_crop.py --no-grid
...
# Fixed paths for croma_m_SA_crop_type
BASE_OUTPUT_DIR = Path('/mnt/tania/geobreeze/outputs/croma_m_SA_crop_type')
CHECKPOINT_PATH = BASE_OUTPUT_DIR / 'lr=3.125e-06_bsz=8_e=50/checkpoints/best_model-epoch=49.ckpt'
...
def load_lightning_task(checkpoint_path: str, device: str):
    ...
    from geobreeze.models.croma import Croma
    ...
    # Build CROMA encoder using saved config
    model_cfg = cfg.model
    logger.info(f"Building CROMA encoder: {model_cfg.modality} modality, {model_cfg.size} size")
    try:
        encoder = Croma(
            ...
            hf_filename=model_cfg.hf_filename,
            size=model_cfg.size,
        )
    except Exception as e:
        logger.warning(f"Error building encoder: {e}, will try loading state dict")
        encoder = None

    # Load checkpoint with encoder passed in, strict=False to handle missing keys
    try:
        ...
        # Try loading without encoder parameter if strict instantiation fails
        # The state dict will be loaded anyway
        task = LightningSegmentationTask.load_from_checkpoint(
            checkpoint_path,
            map_location=device,
            strict=False,
...
def plot_sample_detailed(rgb, gt, pred, confidence, sample_id, out_path):
    ...
    fig.suptitle(f'South Africa Crop Type Sample {sample_id} · CROMA Prediction',
...
def plot_comparison_grid(samples, out_path):
    ...
    fig.suptitle('South Africa Crop Type · CROMA Predictions (GT vs Pred)',
...
def plot_class_performance(all_ious, all_f1s, out_path):
    ...
    fig.suptitle('Per-Class Performance (CROMA on South Africa Crop Type)',
...
def plot_confusion_matrix(all_gt, all_pred, out_path):
    ...
    ax.set_title('Confusion Matrix (CROMA on South Africa Crop Type)', 
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualize CROMA South Africa Crop Type model predictions',
    ...
    try:
        ...
        # Apply resizing transform for test split
        test_transforms = [
